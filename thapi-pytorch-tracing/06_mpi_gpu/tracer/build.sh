...
#
#   source <env-skill>/scripts/env.sh lttng    # torch + LTTng tooling (lttng-gen-tp)
...
# torch's CXX11 ABI (=1) or the callback ABI is wrong (ENTER/EXIT desync,
# fn.inputs() garbage). This has bitten the project before.
set -euo pipefail
cd "$(dirname "$0")"

: "${TORCH_INCLUDE:?source the env skill's env.sh with 'lttng' first (TORCH_INCLUDE unset)}"
: "${TORCH_LIB:?source the env skill's env.sh with 'lttng' first (TORCH_LIB unset)}"
command -v lttng-gen-tp >/dev/null 2>&1 || { echo "lttng-gen-tp missing: source env.sh lttng"; exit 1; }
