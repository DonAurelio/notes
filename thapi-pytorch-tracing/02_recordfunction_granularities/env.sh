# Canonical environment for the PyTorch tracing project (Aurora).
# Source this at the top of every script / remote command — the module
# environment does NOT propagate across shells or to compute nodes.
#
#   source scripts/env.sh            # torch only (build the tracer, run workloads)
#   source scripts/env.sh lttng      # torch + LTTng tooling (record/view traces)
#
